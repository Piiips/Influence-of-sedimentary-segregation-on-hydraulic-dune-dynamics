...
q_seg = 0.001        # Gravity-driven segregation velocity [m/s] (stronger for visible sorting)
...
plt.subplots_adjust(bottom=0.18, top=0.88, left=0.06, right=0.95)
...
cbar.set_label(r"Concentración de Finos $\phi_s$ (Blanco=grueso, Rojo=fino)", fontsize=10, fontweight='bold')
...
for f_count in range(int(t_max / dt) + 10):
    ...
    if t_current >= next_frame_time:
        ...
        ax.text(0.02, 0.88, f"Tiempo: {t_current:.1f} s\nCresta x: {x_start_lab + x_crest_offset:.3f} m",
