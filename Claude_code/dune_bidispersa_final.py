...
# perfil triangular h(x) = H_base + H_d·f(x)
h = np.where(xc <= x_crest,
             H_base + H_d * (xc / x_crest),
             H_base + H_d * (1 - (xc - x_crest) / L_lee))
dh = np.where(xc <= x_crest, H_d / x_crest, -H_d / L_lee)
...
def rhs_horiz(Q_in, t_val):
    ...
    Esquema upwind de primer orden vectorizado.
    """
    g_t = 1.0 + A_mod * np.sin(2*np.pi*t_val/T_period) * Sx
    u   = (U_0*H_base/h2) * (m_exp+1) * Ec**m_exp * g_t[:,None] - c_mig
    # cara x_{i-1/2}: promedio de velocidades adyacentes
    uf  = 0.5 * (u + np.roll(u, 1, axis=0))
    Ql  = np.roll(Q_in, 1, axis=0)
    Fx  = np.where(uf >= 0, uf * Ql, uf * Q_in)
    return -(np.roll(Fx, -1, axis=0) - Fx) / dx


def rhs_vert(Q_in, w_f, gd, Pt):
    """
    Parte vertical rígida (sub-ciclada):
      − ∂/∂η[w_η·φ_s  −  f_sl·φ_s·(1−φ_s)  −  D_sl/h·∂φ_s/∂η]

    i) Advección vertical + condición de superficie (deposición/erosión)
       El balance de masa stoss↔lee reproduce el ciclo de Kleinhans (2004):
       todo lo que se erosiona en el stoss se deposita en el lee, con
       sorting espacial P_spat (grueso→pie, fino→cresta) y modulación
       temporal P_temp (frecuencia de grain-flows).

    ii) Segregación (Trewhela 2021): finos decantan hacia la base.
        F_seg = −f_sl · φ_s(j+1) · (1−φ_s(j))   [upwind: aguas arriba = j+1]

    iii) Difusión (Trewhela 2021): amortigua los gradientes verticales.
         F_diff = D_sl/h · (φ_s(j+1)−φ_s(j)) / dη
    """
    phi = Q_in / h2                              # (Nx, Nz)
    ...
    Fv = np.zeros((Nx, Nz+1))
    wp = w_f >= 0
    Fv[:, 1:-1] = np.where(wp[:, 1:-1], w_f[:, 1:-1]*phi[:,:-1],
                                         w_f[:, 1:-1]*phi[:, 1:])
    # base η=0: impermeable
    Fv[:, 0] = 0.0
    ...
    # ── (ii+iii) Segregación + Difusión (Trewhela et al. 2021) ─────────
    # φ en las caras η (promedio centrado, paredes = valor de celda)
    pf       = np.empty((Nx, Nz+1))
    pf[:, 1:-1] = 0.5 * (phi[:, 1:] + phi[:, :-1])
    pf[:, 0] = phi[:, 0];  pf[:, -1] = phi[:, -1]

    dbar = (1 - pf)*d_l + pf*d_s                # diámetro medio local
    Ffac = (R-1) + E_seg*(1-pf)*(R-1)**2         # asimetría F(R,φ_s)
    f_sl = (B_seg * gd * dbar**2) / (C_seg*dbar + p_face_st) * Ffac
    D_sl = A_diff * gd * dbar**2

    # segregación (upwind: finos bajan → aguas arriba es j+1)
    Fs = np.zeros((Nx, Nz+1))
    Fs[:, 1:-1] = -f_sl[:, 1:-1] * phi[:, 1:] * (1 - phi[:, :-1])

    # difusión (Fick centrado)
    Fd = np.zeros((Nx, Nz+1))
...
dt_h   = 0.8 * dx / u_max                       # paso grande (horizontal)
...
dt_v_adv  = 0.8 * deta * np.min(h) / max(w_eta_max + f_sl_max, 1e-15)
...
# =============================================================================
target_t   = [0.0, 60.0, 120.0, 180.0, 300.0]
...
fps_vid = 20
vp  = os.path.join(out_dir, "duna_bidispersa_final.mp4")
...
fig_v.savefig(os.path.join(out_dir, "duna_bidispersa_final.png"), dpi=300)
...
np.savez(os.path.join(out_dir, "snapshots_final.npz"),
...
fp = os.path.join(out_dir, "fan_diagram_final.png")
