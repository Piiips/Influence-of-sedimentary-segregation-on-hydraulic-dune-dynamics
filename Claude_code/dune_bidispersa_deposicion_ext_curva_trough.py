...
# En vez de pegar el pie del lee directo con el pie del stoss, se inserta lecho
# PLANO a la altura base H_base. Así el "trough" entre dunas se resuelve
# explícitamente: los gruesos que ruedan al pie del lee forman un lag basal que
# reposa en el lecho plano y recircula hasta que el siguiente stoss lo re-erosiona.
# Trough a AMBOS lados de la duna: así la costura periódica (x=L_dom↔x=0) queda
# EN MEDIO del lecho plano (ambos lados a H_base, misma composición del lag que
# recircula) → salto de composición = 0 → sin residual en el pie del lee.
...
# =============================================================================
# Malla sobre el DOMINIO total L_dom (duna + lecho plano). Se mantiene dx igual
# que en la versión sin trough para conservar la resolución de las láminas.
...
# La DUNA se ubica tras el primer tramo de lecho plano, de modo que en t=0 ocupa
# x ∈ [0.1, 0.2] m (los gráficos enfocan esa ventana). Hay trough (lecho plano a
# H_base) ANTES [0, 0.1] y DESPUÉS [0.2, 0.25] de la duna: ahí reposa y recircula
# el lag basal de gruesos, y la costura periódica cae en el lecho plano.
...
# duna solo donde 0 ≤ xd ≤ L_dune; resto = lecho plano a H_base
...
# ── OPCIÓN 2 (regularización del empalme): se suaviza levemente la pendiente
#    que alimenta la velocidad vertical ω_code, para que NO cambie abruptamente
#    en el empalme lee-toe↔trough (donde dh/dx salta de −tan30° a 0). Así se
#    evitan los contornos comprimidos/oscilantes en ese junction. La forma h que
#    se DIBUJA queda igual; solo se regulariza la dh que entra en w_η.
...
print(f"  [geom. curva + trough + fix] p_stoss={p_stoss}  L_dom={L_dom*1e3:.0f}mm "
      f"Nx={Nx}  duna∈[{x_dune0*1e3:.0f},{x_lee_toe*1e3:.0f}]mm  "
      f"pend. máx lee: {_alpha_lee_loc:.2f}°")
...
# ── OPCIÓN 1 (atenuación de la deposición hacia el pie del lee): una ventana
#    sigmoide W_dep ≈1 en la cresta y el lee medio, y →0 en el último tramo del
#    lee (s_lee→1). Así la deposición se apaga SUAVEMENTE antes de llegar al
#    empalme con el lecho plano, en vez de cortarse en seco: elimina la
#    inyección abrupta de composición que generaba la "zona roja de la nada".
...
fps_vid = 20
vp  = os.path.join(out_dir, "duna_bidispersa_deposicion_ext_curva_trough.mp4")
...
while t_cur < t_max:
    ...
    if t_cur >= next_frame:
        ...
        # perfil de la duna (línea de superficie)
        ax_v.plot(x_lab, h, 'k-', lw=2.0, zorder=4)
        ax_v.fill_between(x_lab, 0, h, color='none', zorder=0)

        # ── ENFOQUE SOLO EN LA DUNA (recorta el lecho plano) ──────────────────
        # ventana = tramo de la duna [x_dune0, x_lee_toe] siguiendo la migración;
        # en t=0 es [0.1, 0.2] m.
...
fig_v.savefig(os.path.join(out_dir, "duna_bidispersa_deposicion_ext_curva_trough.png"), dpi=300)
...
np.savez(os.path.join(out_dir, "snapshots_deposicion_ext_curva_trough.npz"),
...
for idx, tt in enumerate(target_t):
    ...
    ax.text(0.01, 0.72, f"t = {int(tt)} s", transform=ax.transAxes,
            ...
            bbox=dict(fc='white', alpha=0.8, ec='none', boxstyle='round,pad=0.2'))
    # ENFOQUE SOLO EN LA DUNA (recorta el lecho plano); en t=0 → [0.1, 0.2] m
...
fp = os.path.join(out_dir, "fan_diagram_deposicion_ext_curva_trough.png")
