...
# =============================================================================
def figura_comparacion(Md, Mh, Dd, Dh, out_dir):
    ...
    fig_ab, (ax1, ax2) = plt.subplots(2, 1, figsize=(8, 4.5), sharex=True, sharey=True)
    
    paneles_ab = [
        (ax1, phi_d, '(a) CON difusión  ($A$=%.3f)' % Md.A_diff),
        (ax2, phi_h, r'(b) SIN difusión  ($Pe_{fis}\to\infty$)')
    ]
    
    for ax, ph, ttl in paneles_ab:
        pm = ax.pcolormesh(X, Z, ph[sl], cmap=A.CMAP_PHI, vmin=0, vmax=1, shading='gouraud', rasterized=True)
        ax.contour(X, Z, ph[sl], levels=np.linspace(0.1, 0.9, 9), colors='k', linewidths=0.35, alpha=0.5)
        ...
        ax.set_ylabel(r'$z$ (mm)', fontsize=10)
        ax.set_title(ttl + f'   $t$ = {t_fin:.0f} s', fontsize=11, loc='left')
        ax.tick_params(direction='in', top=True, right=True)
        ax.set_aspect(2, adjustable='box')
        
    ax1.invert_xaxis()
    ax2.set_xlabel(r'$x$ (mm)', fontsize=10)
    
    cb = fig_ab.colorbar(pm, ax=[ax1, ax2], orientation='horizontal', fraction=0.06, pad=0.15)
    cb.set_label(r'$\phi_s$', fontsize=10)
    
    out_path = os.path.join(out_dir, 'panel_ab_combinado.png')
    fig_ab.savefig(out_path, dpi=250, facecolor='white', bbox_inches='tight', pad_inches=0.02)
    plt.close(fig_ab)
    print("  ✓ panel_ab_combinado.png")

    # ── (c) diferencia ──
    dif = phi_d - phi_h
    vm_max = float(np.nanmax(np.abs(dif[sl])))
    vm = float(np.nanpercentile(np.abs(dif[sl]), 90))   # 90% de los puntos dentro
    fig, axC = plt.subplots(figsize=(8, 4))
    pmc = axC.pcolormesh(X, Z, dif[sl], cmap='RdBu_r', vmin=-vm, vmax=vm, shading='gouraud', rasterized=True)
    axC.contour(X, Z, dif[sl], levels=[-0.3, -0.15, 0.15, 0.3], colors='k', linewidths=0.4, alpha=0.4)
    axC.plot(xmm, M.h[sl] * 1e3, 'k-', lw=1.6)
    axC.plot(xmm, (M.h[sl] - M.delta_a) * 1e3, color='0.35', ls=':', lw=1.2, label=r'$\eta_a$ (base capa activa)')
    axC.set_xlabel(r'$x$ (mm)', fontsize=10); axC.set_ylabel(r'$z$ (mm)', fontsize=10)
    axC.set_title(r'(c) $\Delta\phi_s=\phi_s^{\rm con\,dif.}-\phi_s^{\rm sin\,dif.}$'
                  '   (mismo punto, mismo $t$)', fontsize=11, loc='left')
    axC.legend(fontsize=8, loc='upper left', framealpha=0.9)
    axC.tick_params(direction='in', top=True, right=True)
    axC.invert_xaxis()
    axC.set_aspect(2, adjustable='box')
    cbc = fig.colorbar(pmc, ax=axC, orientation='horizontal', fraction=0.08, pad=0.25)
    cbc.set_label(r'$\Delta\phi_s$', fontsize=10)
    
    rms = np.sqrt(np.mean(dif[sl]**2))
    
    # Escribir información extraída a un archivo Markdown
    info_md_path = os.path.join(out_dir, 'info_panel_c.md')
    with open(info_md_path, 'w', encoding='utf-8') as f:
        f.write("# Información del Panel C (Diferencia)\n\n")
        f.write("- **Leyenda de Colores**:\n")
        f.write("  - **Rojo** ($\\Delta\\phi_s>0$): Con difusión hay MÁS FINOS aquí.\n")
        f.write("  - **Azul** ($\\Delta\\phi_s<0$): Con difusión hay MÁS GRUESOS aquí.\n\n")
        f.write("- **Métricas de Diferencia**:\n")
        f.write(f"  - **RMS**: {rms:.3f}\n")
        f.write(f"  - **Máx |Δ|**: {vm_max:.3f}\n")
        f.write(f"  - **Escala saturada a**: ±{vm:.2f}\n")
        f.write(f"  - **|Δ| > 0.05 en el**: {100*np.mean(np.abs(dif[sl]) > 0.05):.0f}% de la duna\n")
    
    out_path = os.path.join(out_dir, 'panel_c_diferencia.png')
    fig.savefig(out_path, dpi=250, facecolor='white', bbox_inches='tight', pad_inches=0.02)
    plt.close(fig)
    print("  ✓ panel_c_diferencia.png")

    # ── (d) perfiles φ_s(η) superpuestos ──
    fig, axD = plt.subplots(figsize=(6, 6))
    ...
    axD.set_xlabel(r'$\phi_s$', fontsize=10); axD.set_ylabel(r'$\eta = z/h$', fontsize=10)
    axD.set_title(r'(d) perfiles verticales — —— con difusión, - - - sin difusión'
                  '\n' r'      (marcas $\blacktriangleleft$: $\eta_a$ de cada estación)',
                  fontsize=10, loc='left')
    axD.legend(fontsize=8, loc='lower left', ncol=2, framealpha=0.9)
    axD.tick_params(direction='in', top=True, right=True)
    fig.tight_layout()
    out_path = os.path.join(out_dir, 'panel_d_perfiles.png')
    fig.savefig(out_path, dpi=250, facecolor='white')
    plt.close(fig)
    print("  ✓ panel_d_perfiles.png")
    ...
    axE.set_ylabel(r'$\delta_{int}$ (mm)', fontsize=10)
    axE.set_yscale('log')
    ...
    axE.legend(fontsize=9, loc='upper right', framealpha=0.92, ncol=2)
    axE.tick_params(direction='in', top=True, right=True, which='both')
    
    # Textos de cierre (movidos desde el panel F eliminado)
    ...
    axE.text(0.985, 0.05,
             ...
             transform=axE.transAxes, ha='right', va='bottom', fontsize=9,
             bbox=dict(fc='#fff8e1', ec='0.6', boxstyle='round,pad=0.3'))

    fig.tight_layout()
    out_path = os.path.join(out_dir, 'panel_e_interfase.png')
    fig.savefig(out_path, dpi=250, facecolor='white')
    plt.close(fig)
    print("  ✓ panel_e_interfase.png")
...
if __name__ == "__main__":
    ...
    figura_comparacion(Md, Mh, Dd, Dh, od)
    print("══ COMPLETADO ══")
