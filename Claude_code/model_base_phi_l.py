...
================================================================================
model_base_phi_l.py
Duna bidispersa — CORRIDA ÚNICA con UNA pendiente, formulada en φ_l (GRUESOS)
Concentración ÚNICA: φ_l = 0.3   (equivalente a φ_s = 0.7 del modelo base)
================================================================================
Versión de `Single_slope_model.py` reformulada en la CONCENTRACIÓN DE PARTÍCULAS
GRANDES φ_l en lugar de la de finos φ_s, para comparar si el procedimiento
numérico da resultados distintos según desde qué especie se mire.

El modelo base resuelve la ec. (2.4) (especie pequeña):
    ∂φ_s/∂t + ∇·(φ_s u) + ∇·(f_sl φ_s φ_l g/|g|) = ∇·(D_sl ∇φ_s)
Este script resuelve la ec. (2.3) (especie grande):
    ∂φ_l/∂t + ∇·(φ_l u) − ∇·(f_sl φ_l φ_s g/|g|) = ∇·(D_sl ∇φ_l)

Cambios respecto de `Single_slope_model.py` (todo lo demás es idéntico):
  1. La variable conservada es h_phi = h(x)·φ_l(x,η,t), con φ_s = 1 − φ_l.
  2. El flujo de segregación vertical cambia de signo: los GRUESOS SUBEN,
       F_seg = +f_sl·φ_l·(1−φ_l)   (antes: −f_sl·φ_s·(1−φ_s), finos bajan).
  3. Ley de Trewhela et al. (2021) reescrita en φ_l:
       d̄   = φ_l·d_l + (1−φ_l)·d_s          (antes: (1−φ_s)·d_l + φ_s·d_s)
       F(R) = (R−1) + E·φ_l·(R−1)²           (antes: E·(1−φ_s)·(R−1)²)
     — son las MISMAS expresiones físicas con φ_s = 1−φ_l.
  4. Condición de borde de deposición en la superficie espejada:
       espacial:  P_spat_l = 1 + sort_slope·(s_lee − 0.5)  (grueso → pie del lee)
       temporal:  Pt_l     = 1 − A_P·sign(sin)·|sin|^p     (pulso de gruesos en
                                                            contrafase al de finos)
     y el cierre de masa λ conserva ahora la masa de GRUESOS.
  5. Colormap invertido para conservar el código visual del modelo base:
     ROJO = fino (φ_l→0), BLANCO = grueso (φ_l→1).

La velocidad de corte es un PERFIL LOCAL, no una constante:
    U_shear(x) = sqrt(g·h(x)·i_slope)

Salidas (sufijo `_phil030`; se sobrescriben en cada corrida):
  · Video               model_base_phi_l_phil030.mp4
  · Estado final (PNG)  model_base_phi_l_phil030.png
  · Diagrama de abanico model_base_phi_l_fan_phil030.png
  · Snapshots (.npz)    model_base_phi_l_snapshots_phil030.npz

FÍSICA: PDE de Gray & Chugunov (2006) / Trewhela, Ancey & Gray (2021):
    h_phi(x,η,t) = h(x)·φ_l(x,η,t),   η = z/h(x) ∈ [0,1]
    d(h_phi)/dt + ∂/∂x(u·h_phi) + ∂/∂η[w_η·φ_l + f_sl·φ_l·(1−φ_l) − D_sl/h·∂φ_l/∂η] = 0
...
PHI_L = 0.3         # fracción volumétrica de GRUESOS de la duna (= 1 − φ_s del modelo base)
g = 9.81 #m/s2
...
demo_speedup = 1#10.0     # [-] factor de aceleración SOLO para visualizar la laminación.
...
E_seg   = 2.0957    # [-] coeficiente de asimetría F(R,φ) = (R−1) + E·φ_l·(R−1)²
...
# ESPEJADO respecto del modelo base: la preferencia de deposición de GRUESOS
# crece hacia el pie del lee (s_lee=1) y decae hacia la cresta (s_lee=0).
s_lee  = np.clip((xc - x_crest_abs) / L_lee, 0, 1)   # 0 en cresta, 1 en pie del lee
P_spat = np.maximum(0.1, 1.0 + sort_slope * (s_lee - 0.5))  # (Nx,)
W_dep  = 0.5 * (1.0 - np.tanh((s_lee - s_taper) / w_taper)) # (Nx,) apagado al pie del lee
P_spat = P_spat * W_dep
# presión granular litostática (independiente de φ_l y t) + piso de regularización
...
def precompute(t_val, U_shear_x):
    """Coeficientes que dependen de t pero NO de φ_l (se congelan en el sub-ciclo).
    ...
    # pulso de deposición de GRUESOS en contrafase al pulso de finos del modelo
    # base: cuando el modelo base deposita más finos (Pt_s>1), aquí Pt_l<1.
...
def rhs_vert(h_phi_in, w_f, gd, Pt):
    """Parte vertical rígida (sub-ciclada) — MUSCL + Rusanov:
       − ∂/∂η[w_η·φ_l + f_sl·φ_l·(1−φ_l) − D_sl/h·∂φ_l/∂η].
    El flujo de segregación es POSITIVO (hacia η=1): los gruesos suben
    por squeeze expulsion (ec. 2.3), en contraste con −f_sl·φ_s·(1−φ_s)
    de la ec. 2.4 del modelo base (los finos percolan hacia abajo)."""
    phi = h_phi_in / h2
...
# =============================================================================
# 5. CORRIDA ÚNICA  (φ_l = PHI_L, pendiente i_slope)
# =============================================================================
def run_simulation():
    """Simulación completa (loop + video + fan diagram + snapshots) para
    φ_l=PHI_L y la pendiente i_slope, con U_shear(x)=sqrt(g·h(x)·i_slope)."""
    phi_l_target = PHI_L
    tag = f"phil{int(round(phi_l_target*100)):03d}"    # 'phil030'
    pfx = f"[i={i_slope:.5f}, φ_l={phi_l_target:.2f}]"
    ...
    # ── 5.1 CONDICIÓN INICIAL (masa HOMOGÉNEA: φ_l constante en el dominio) ──
    phi_ic = np.full((Nx, Nz), phi_l_target)
    ...
    _db0  = _pf0*d_l + (1-_pf0)*d_s
    _ff0  = (B_seg*_gd0*_db0**2) / (C_seg*_db0 + p_face_st) * ((R-1)+E_seg*_pf0*(R-1)**2)
    ...
    # colormap invertido respecto del base: ROJO = fino (φ_l→0), BLANCO = grueso (φ_l→1)
    cmap_phi = mcolors.LinearSegmentedColormap.from_list(
        'red_white', [(0.78,0.06,0.08), (1.0,1.0,1.0)], N=256)
    ...
    cb.set_label(r"$\phi_l$ — fracción de gruesos  (rojo=fino, blanco=grueso)", fontsize=9)
    ...
    while t_cur < t_max:
        ...
        if step % 3000 == 0:
            ...
            print(f"{pfx} t={t_cur:6.1f}s  step={step:6d}  <φ_l>={mp:.6f}  "
    ...
    cbar2.set_label(r"$\phi_l$ — fracción de gruesos  (rojo=fino, blanco=grueso)", fontsize=9)
    ...
    return {"phi_l": phi_l_target, "i": i_slope,
            "video": vp, "png": png_final,
            "fan": fp, "npz": npz_path, "wall_s": time.time()-t0_wall}


# =============================================================================
# 6. MAIN — una sola corrida (φ_l = 0.3, pendiente i_slope)
# =============================================================================
if __name__ == "__main__":
    print("══════════════════════════════════════════════════════════════")
    print(f"  model_base_phi_l — φ_l = {PHI_L:.2f} (φ_s = {1-PHI_L:.2f}), i = {i_slope:.5f}")
    ...
    print("  Modelo en φ_l (ec. 2.3): adv.+segregación(+η)+difusión (Trewhela 2021)")
    print("══════════════════════════════════════════════════════════════", flush=True)
    ...
    print("\n══ RESUMEN ══")
    print(f"  i={r['i']:.5f}  φ_l={r['phi_l']:.2f}")
