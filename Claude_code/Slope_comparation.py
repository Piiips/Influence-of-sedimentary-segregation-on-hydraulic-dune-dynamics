"""
================================================================================
Slope_comparation.py
Duna bidispersa — COMPARACIÓN ENTRE PENDIENTES (i1 vs i2)
Concentración ÚNICA: φ_s = 0.7
================================================================================
Versión de φ_s única (= 0.7) de
`dune_bidispersa_deposicion_ext_curva_trough_multiphi_parametrico.py`, pensada
para AJUSTAR PARÁMETROS del sistema (c_mig_fisico, U_0, etc.) e ir
comparando el resultado con tus experimentos, sin el costo de barrer 5
concentraciones a la vez.

En esta versión la velocidad de corte es un PERFIL LOCAL, no una constante:
    U_shear(x) = sqrt(g·h(x)·i)
(opción A: h(x) es el perfil completo de la duna, incluido H_base, de modo que
el trough conserva un corte residual). Además, el script corre AMBAS
pendientes hidráulicas i1 e i2 (dict SLOPES) en la misma ejecución,
manteniendo constante todo el resto de los parámetros.
...
Salidas (una tanda por pendiente, sufijos `_i1_phi070` y `_i2_phi070`;
se sobrescriben en cada corrida):
  · Video               Slope_comparation_i1_phi070.mp4          (ídem _i2_)
  · Estado final (PNG)  Slope_comparation_i1_phi070.png
  · Diagrama de abanico Slope_comparation_fan_i1_phi070.png
  · Snapshots (.npz)    Slope_comparation_snapshots_i1_phi070.npz
...
# U_shear ya NO es un escalar: ahora es un PERFIL U_shear(x) = sqrt(g·h(x)·i)
...
# Cambia esto (p. ej. a "Slope_comparation_run3") para NO sobrescribir corridas
# previas y poder comparar experimentos con distintos parámetros lado a lado.
OUT_BASE = "Slope_comparation"
...
# =============================================================================
# 5. CORRIDA POR PENDIENTE  (φ_s = PHI_S, i = i_slope)
# =============================================================================
def run_simulation(slope_name, i_slope):
    """Simulación completa (loop + video + fan diagram + snapshots) para
    φ_s=PHI_S y pendiente i_slope, con U_shear(x)=sqrt(g·h(x)·i_slope)."""
    phi_s_target = PHI_S
    tag = f"{slope_name}_phi{int(round(phi_s_target*100)):03d}"    # 'i1_phi070'
    pfx = f"[{slope_name}={i_slope:.5f}, φ_s={phi_s_target:.2f}]"
