"""
================================================================================
dune_bidispersa_deposicion_ext_curva_trough_multiphi_parametrico.py
Modelo de movimiento interno de una duna bidispersa bajo flujo constante
BARRIDO DE CONCENTRACIÓN: φ_s ∈ {0.5, 0.6, 0.7, 0.8, 0.9}  (5 valores)
================================================================================
Igual a `dune_bidispersa_deposicion_ext_curva_trough_multiphi.py` pero:

  1. Recorre CINCO concentraciones de finos: φ_s = 0.5, 0.6, 0.7, 0.8, 0.9.
  2. TODOS los parámetros ajustables del sistema están reunidos en UN SOLO bloque
     de configuración al inicio (sección « PARÁMETROS AJUSTABLES »), con su
     significado físico, unidades y el efecto que tienen sobre el resultado. La
     idea es que puedas calibrar U_shear, c_mig_fisico, U_0, etc. para reproducir
     tus experimentos SIN tener que tocar nada más del código.

  ►►► SOLO NECESITAS EDITAR LA SECCIÓN 1 (« PARÁMETROS AJUSTABLES »). ◄◄◄
      El resto (malla, solver, video, fan diagram) se deriva automáticamente.

Salidas por cada concentración (sufijo `_phiXXX`, ej. `_phi070`):
  · Video               duna_bidispersa_deposicion_ext_curva_trough_phiXXX.mp4
  · Estado final (PNG)  duna_bidispersa_deposicion_ext_curva_trough_phiXXX.png
  · Diagrama de abanico fan_diagram_deposicion_ext_curva_trough_phiXXX.png
  · Snapshots (.npz)    snapshots_deposicion_ext_curva_trough_phiXXX.npz

FÍSICA (idéntica a la versión original; ver esa docstring para el detalle):
...
Las 5 corridas son independientes → se ejecutan EN PARALELO (una por proceso).
================================================================================
"""
...
# Los valores por defecto son los de la versión original (φ_s barrido en 0.5–0.9).

# ── 1.1 CONCENTRACIONES A BARRER ─────────────────────────────────────────────
PHI_S_LIST = [0.5, 0.6, 0.7, 0.8, 0.9]   # fracción volumétrica de finos por corrida
...
# ── 1.11 PARALELISMO ─────────────────────────────────────────────────────────
# nº de corridas simultáneas (None = min(nº de φ_s, nº de núcleos de la CPU)).
N_WORKERS = None
...
# =============================================================================
# 3. MALLA EN COORDENADAS σ (marco co-móvil)  — INDEPENDIENTE de φ_s
# =============================================================================
...
# 4. NÚCLEO DEL SOLVER
# =============================================================================
# PHI_S_TARGET se fija por corrida (un valor por proceso). Solo lo usa rhs_vert
# como composición de respaldo cuando no hay capacidad de depósito.
...
# =============================================================================
# 5. UNA CORRIDA COMPLETA PARA UN VALOR DE φ_s
# =============================================================================
def run_simulation(phi_s_target):
    """Simulación completa (loop + video + fan diagram + snapshots) para una
    única concentración φ_s. Devuelve un dict resumen."""
    global PHI_S_TARGET
    PHI_S_TARGET = phi_s_target

    tag = f"phi{int(round(phi_s_target*100)):03d}"    # ej. 'phi070'
    ...
    print(f"{pfx} ═══ INICIO ═══  R=d_l/d_s={R:.2f}  H={H_d*1e3:.1f}mm "
          ...
          f"U_shear={U_shear*1e3:.1f}mm/s  U_0={U_0*1e3:.2f}mm/s", flush=True)
    ...
    # ── 5.2 PASO DE TIEMPO (dt_v depende de f_sl(φ_ic) → por corrida) ──
    ...
    fig_f.suptitle(
        f"Estructura interna de la duna bidispersa  ($\\phi_s$={phi_s_target:.2f}, "
        f"$d_l$={d_l*1e3:.1f} mm, $d_s$={d_s*1e3:.1f} mm)\n"
        r"Adv. + Segregación ($f_{sl}$) + Difusión ($D_{sl}$) — Trewhela, Ancey & Gray (2021)  |  "
...
# =============================================================================
# 6. MAIN — las 5 concentraciones se corren EN PARALELO (una por proceso)
# =============================================================================
if __name__ == "__main__":
    ...
    print("  Adv.+segregación+difusión (Trewhela 2021)  |  corridas en paralelo")
    print("══════════════════════════════════════════════════════════════", flush=True)

    t0 = time.time()
    n_workers = N_WORKERS or min(len(PHI_S_LIST), os.cpu_count() or 1)
