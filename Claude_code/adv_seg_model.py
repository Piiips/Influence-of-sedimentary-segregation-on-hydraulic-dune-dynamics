...
================================================================================
adv_seg_model.py
Duna bidispersa — MODELO PURO DE ADVECCIÓN + SEGREGACIÓN (sin difusión)
Concentración ÚNICA: φ_s = 0.7
================================================================================
Variante de `test_exp_deposicion.py` en la que la PDE se reduce ÚNICAMENTE a
los términos de ADVECCIÓN y SEGREGACIÓN, eliminando por completo la difusión
granular D_sl. Es el modelo hiperbólico puro de Gray & Thornton (2005),
con la velocidad de segregación f_sl calibrada según la ley de escala
experimental de Trewhela, Ancey & Gray (2021):

    Q(x,η,t) = h(x)·φ_s(x,η,t),   η = z/h(x) ∈ [0,1]
    dQ/dt + ∂/∂x(u·Q) + ∂/∂η[w_η·φ_s − f_sl·φ_s·(1−φ_s)] = 0

Sin el término −D_sl/h·∂φ_s/∂η la ecuación es puramente hiperbólica: las
interfaces finos/gruesos son CHOQUES de concentración nítidos en vez de
transiciones difusas. El flujo de Rusanov + limitador minmod del esquema
MUSCL capturan esos choques sin oscilaciones.
...
Salidas (sufijo `_phi070`; se sobrescriben en cada corrida):
  · Video               adv_seg_model_phi070.mp4
  · Estado final (PNG)  adv_seg_model_phi070.png
  · Diagrama de abanico adv_seg_model_fan_phi070.png
  · Snapshots (.npz)    adv_seg_model_snapshots_phi070.npz
...
demo_speedup = 10.0     # [-] factor de aceleración SOLO para visualizar la laminación.
                        ...
                        #     (f_sl, avalanchas) NO cambia, solo qué tan rápido rota el volumen.
...
U_shear = np.sqrt(g*H_d*i1)     # [m/s] escala de CORTE de la capa activa (impulsa el kinetic sieving).
                    #       DESACOPLADA de la advección: sube/baja la INTENSIDAD de segregación
                    #       sin cambiar la velocidad de migración. ↑ → láminas más contrastadas.
...
# ajustas el modelo a tu material.  NOTA: en este modelo NO hay difusión (A_diff
# no existe): la única física vertical además de w_η es el flujo de segregación.
...
p_floor_factor = 3.0  # [-] piso de presión granular = ν·(p_floor_factor·δ_a). ↑ → f_sl menos rígido
...
cfl_vert_adv = 0.5      # [-] CFL del sub-paso vertical (advección + segregación)
...
# Cambia esto (p. ej. a "adv_seg_run3") para NO sobrescribir corridas previas
# y poder comparar experimentos con distintos parámetros lado a lado.
OUT_BASE = "adv_seg_model"
...
def rhs_vert(Q_in, w_f, gd, Pt):
    """Parte vertical (sub-ciclada) — MUSCL + Rusanov, SOLO advección+segregación:
       − ∂/∂η[w_η·φ_s − f_sl·φ_s·(1−φ_s)]   (sin término difusivo)."""
    phi = Q_in / h2
    ...
    # (ii) Segregación (Gray & Thornton 2005; f_sl de Trewhela 2021) — Rusanov
    pf       = np.empty((Nx, Nz+1))
    ...
    dF = ((Fv[:, 1:]-Fv[:, :-1]) + (Fs[:, 1:]-Fs[:, :-1])) / deta
    return -dF


# =============================================================================
# 5. CORRIDA ÚNICA  (φ_s = PHI_S)
# =============================================================================
def run_simulation():
    """Simulación completa (loop + video + fan diagram + snapshots) para φ_s=PHI_S."""
    phi_s_target = PHI_S
    tag = f"phi{int(round(phi_s_target*100)):03d}"    # 'phi070'
    pfx = f"[φ_s={phi_s_target:.2f}]"
    ...
    print(f"{pfx} ═══ INICIO ═══  R=d_l/d_s={R:.2f}  H={H_d*1e3:.1f}mm "
          ...
          f"U_shear={U_shear*1e3:.1f}mm/s  U_0={U_0*1e3:.2f}mm/s  t_max={t_max:.0f}s", flush=True)
    ...
    dt_v      = cfl_vert_adv * deta * np.min(h) / max(w_eta_max + f_sl_max, 1e-15)
    n_sub     = max(1, int(np.ceil(dt_h / dt_v)))
    dt_v_eff  = dt_h / n_sub

    print(f"{pfx} Malla Nx={Nx} Nz={Nz}  M0={M0:.6e}  dt_h={dt_h:.3e}s "
          f"n_sub={n_sub}  f_sl_max={f_sl_max*1e3:.3f}mm/s  (sin difusión)",
          flush=True)
    ...
    while t_cur < t_max:
        ...
        if t_cur >= next_frame:
            ...
            ax_v.set_title(
                ...
                f"$d_s$={d_s*1e3:.1f} mm, R={R:.2f})  —  "
                f"t = {t_cur:.0f} s  ({Tseg:.1f} ciclos de grain-flow)\n"
                r"Advección + Segregación ($f_{sl}$) pura, sin difusión — "
                r"Gray & Thornton (2005) / Trewhela, Ancey & Gray (2021)",
                fontsize=10, fontweight='bold')

            ax_v.text(0.01, 0.80,
                      f"cresta x = {(x_crest_abs + c_mig*t_cur)*1e3:.1f} mm\n"
                      f"c_mig = {c_mig*1e3*60:.2f} mm/min\n"
                      f"U_shear = {U_shear*1e3:.1f} mm/s\n"
...
# =============================================================================
# 6. MAIN — una sola corrida (φ_s = 0.7)
# =============================================================================
if __name__ == "__main__":
    print("══════════════════════════════════════════════════════════════")
    print(f"  adv_seg_model — corrida única φ_s = {PHI_S:.2f}")
    print(f"  U_shear={U_shear*1e3:.1f}mm/s  c_mig_fis={c_mig_fisico*1e3*60:.3f}mm/min  "
          f"demo×{demo_speedup:.0f}  U_0={U_0*1e3:.2f}mm/s  t_max={t_max:.0f}s")
    print("  Advección + segregación PURA (sin difusión) — Gray & Thornton (2005)")
    print("══════════════════════════════════════════════════════════════", flush=True)
