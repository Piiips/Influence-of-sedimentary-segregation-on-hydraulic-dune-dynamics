...
def generar_Figura2():
    ...
    for exp in exps:
        ...
        x_crest = x_mm[icrest]
        # Aguas abajo es hacia la izquierda, por ende el leeside es x <= x_crest
    ...
    # -------------------------------------------------------------
    # DERECHA (ax2): F1_a del modelo (difusión) enfocado en el leeside
    # -------------------------------------------------------------
    ETA_A = float(np.mean(A.eta_active_layer(M)[DUNE]))
    kind = 'dif'
    
    # Redefinir perfil_x_medio localmente para el leeside del modelo
    # El flujo en el modelo va hacia la derecha, el leeside está entre x_crest_abs y x_lee_toe
    def perfil_x_medio_leeside(phi):
        x_crest_abs = M.xc[np.argmax(M.h)]
        mask_lee = (M.xc >= x_crest_abs) & (M.xc <= M.x_lee_toe)
        w = M.h[mask_lee][:, None]
        return (phi[mask_lee] * w).sum(axis=0) / w.sum()
    
    for k, p0 in enumerate(PHIS):
        ph, _ = _load(kind, p0)
        pr = perfil_x_medio_leeside(ph)
        # Línea de simulación
    ...
    out_filename = os.path.join(OUTPUTS_DIR, 'Figura2_leeside.png')
